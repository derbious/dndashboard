...
$(function () {
  ...
  $("#donorSearchForm").submit(function(event) {
    ...
    // make an api call
    $.ajax({
      url: DNDAPI_ENDPOINT+"/search?q="+searchstr,
      type: 'get',
      headers: {
        'Content-Type': 'application/json',
    ...
    $.ajax({
        ...
        success: function(e){
          ...
          for(i=0; i<e['waiting'].length; i++){
            ...
            row += '<div class="row">';
            row += '<p class="col-1 h2">'+e['waiting'][i]['queue_pos']+'</p>';
  ...
  reorderQueue = function(id, to_pos){
    ...
    $.ajax({
      url: DNDAPI_ENDPOINT+"/queue/reorder/",
      type: 'post',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': "JWT " + token
  ...
  removeQueue = function(id){
    if(confirm('Remove character from queue?')){
      ...
      //issue json call to reorder the queue
      $.ajax({
        url: DNDAPI_ENDPOINT+"/queue/remove/"+id,
        type: 'post',
        headers: {
          'Authorization': "JWT " + token
        },
  ...
  renderDonorViewPage = function(donor_id){
    ...
    var donorp = $.ajax({
        url: DNDAPI_ENDPOINT+"/donors/"+donor_id,
        headers: {
          'Authorization': "JWT " + token
        },
        dataType: 'json'
    ...
    var donationp = $.ajax({
        url: DNDAPI_ENDPOINT+"/donations/?donor_id="+donor_id,
        headers: {
          'Authorization': "JWT " + token
        },
        dataType: 'json'
    ...
    var characterp = $.ajax({
        url: DNDAPI_ENDPOINT+"/characters/?donor_id="+donor_id,
        headers: {
          'Authorization': "JWT " + token
        },
        dataType: 'json'
  ...
  $('#characterForm').submit(function(event){
    ...
    var token = sessionStorage.getItem('access_token');
    $.ajax({
      url: DNDAPI_ENDPOINT+"/characters/",
      type: 'post',
      data: JSON.stringify(d),
      headers: {
        'Content-Type': 'application/json',
